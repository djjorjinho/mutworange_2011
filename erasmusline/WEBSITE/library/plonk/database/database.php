...
class PlonkDB {
    ...
    public function connect() {
        ...
        // validate connection
        if (!$this->dbHandler)
            throw new Exception("Could not connect to databaserver or access database." . PHP_EOL . mysqli_connect_error());
    }
    ...
    public function execute($query) {
        ...
        if ($result = @mysqli_query($this->dbHandler, $query)) {
            ...
            throw new Exception('There was an error while executing the query ' . $query . PHP_EOL . mysqli_error($this->dbHandler));

        // If it's an INSERT query, return last insertId
        if (strtoupper(substr($query, 0, 6)) == 'INSERT')
            return (int) mysqli_insert_id($this->dbHandler);
    ...
    public function getColumnAsArray($query, $columnIndex = 0) {
        ...
        if ($columnIndex > sizeof($result[0]))
            throw new Exception('The requested $columnIndex does not exist');
...
// make sure the key exists
    ...
    public function getEnumValues($table, $field) {
        ...
        if (!isset($row['Type']))
            throw new Exception('getEnumValues error: the given field does not exist');
        if (strtolower(substr($row['Type'], 0, 4) != 'enum'))
            throw new Exception('getEnumValues error: ' . (string) $field . ' isn\'t an ENUM field.');
    ...
    public function getNumRows($query) {
        ...
        // catch error
        if (mysqli_error($this->dbHandler) != '')
            throw new Exception(mysqli_error($this->dbHandler));
    ...
    public function getPairsAsArray($query, $columnIndex1 = 0, $columnIndex2 = 1) {
        ...
        if (sizeof($result[0]) < 2)
            throw new Exception('Could not complete getPairsAsArray as the query returned too little (minimum 2 required) columns.');
...
// make sure the key exists
        if (!in_array($keyC1, $keys) || !in_array($keyC2, $keys))
            throw new Exception('The requested $columnIndex does not exist');

        // build toReturn
        foreach ($result as $row)
            $toReturn[$row[$keyC1]] = $row[$keyC2];
