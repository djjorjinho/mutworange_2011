...
require_once './core/layout/main/main.php';
...
class PlonkController {
    ...
    final private function defineAction() {
        ...
        if (PlonkFilter::getPostValue('formAction') !== null) {
            ...
            } else if (PlonkFilter::startsWith($formAction, 'main')) {
                $mainAction = strtolower(substr($formAction, 4));
                MainController::doFunction($mainAction);
            } else {
    ...
    final private function display() {
        ...
        //Values which are the same in the whole website but can't be static can be kept in config file or global values file and then used here to fill main
            
        MainController::assignMain();
    ...
    final public function execute() {
        ...
        if (sizeof($this->views) === 0)
            throw new Exception('Cannot initialize module "' . MODULE . '": no views defined; define at least one view');
    ...
    final private function loadPageTemplate() {
        ...
        if (!file_exists(PATH_MODULES . '/' . MODULE . '/layout/' . $this->view . '.tpl'))
            throw new Exception('Cannot show view "' . $this->view . ' for module ' . MODULE . '": the view tpl file does not exist');
