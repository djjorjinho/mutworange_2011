...
class Thread {
     ...
     */
    public function __construct( $_runnable = null, $_object = null ) {
    	if( $_runnable !== null ) {
        	$this->setRunnable( $_runnable, $_object );
    	}
    }
     ...
     * @return callback
     */
    public function setRunnable( $_runnable, $_object = null ) {
        if( self::runnableOk( $_runnable, $_object ) ) {
            $this->runnable = $_runnable;
            $this->object = $_object;
        }
        else {
            throw new Core_Exception( $this->getError( Thread::FUNCTION_NOT_CALLABLE ), Thread::FUNCTION_NOT_CALLABLE );
        }
    }
    ...
    {
        if($this->object)
            return array($this->object, $this->runnable);
     ...
     * @return boolean
     */
    public static function runnableOk( $_runnable, $_object = null ) {
        if($_object !== null)
            return ( method_exists( $_object, $_runnable ) && is_callable( array($_object, $_runnable ) ) );
        else
    ...
    public function start() {
        $pid = pcntl_fork();
        if( $pid == -1 ) {
            throw new Core_Exception( $this->getError( Thread::COULD_NOT_FORK ), Thread::COULD_NOT_FORK );
        }
        if( $pid ) {
            // parent 
            $this->pid = $pid;
            ...
            if ( !empty( $arguments ) ) {
                call_user_func_array( $this->getRunnable(), $arguments );
            }
            else {
                call_user_func( $this->getRunnable() );
            }
